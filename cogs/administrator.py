...
class Administrator(Cog):
    ...
    @embed_group.command(name="send", description="sends embeds .")
    @guild_only()
    @has_guild_permissions(administrator=True)
    ...
    async def strip_staff(self, ctx, member):
        ...
        emb = Embed(color=0x2b2d31)
        perm_check = ['kick_members', 'ban_members', 'administrator', 'manage_channels', 'manage_guild', 'manage_messages', 'manage_roles', 'manage_webhooks', 'manage_expressions', 'manage_threads', 'moderate_members']
    ...
    async def whitelist_list(self, ctx):
        ...
        if ctx.guild.id not in self.bot.whitelist.keys():
            ...
            emb.set_footer(text=f"{len(cmds)} command{'' if len(cmds) == 1 else 's'} ⋅ {len(usrs)} member{'' if len(usrs) == 1 else 's'}")
        await ctx.send(embed=emb)
