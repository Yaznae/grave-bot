...
def get_prefix(bot, msg):
    with open('./config.json') as f:
        config = json.load(f)
    return when_mentioned_or(str(config['prefix']))(bot, msg)
...
async def on_command_error(ctx, err):
    if isinstance(err, MissingPermissions):
        ...
        emb.add_field(name='usage :', value=f"`{config['prefix']}{ctx.command.qualified_name} {ctx.command.signature}`", inline=False)
        usage = ctx.command.signature.replace('[member]', '@zt').replace('[role]', 'sped').replace('[color]', '#010101').replace('[prefix]', ';').replace('<member>', '@zt').replace('<role>', 'sped').replace('<color>', '#010101').replace('<prefix>', ';').replace('[reason]', 'retarded lol').replace('[duration]', '2m')
        emb.add_field(name='example :', value=f"`{config['prefix']}{ctx.command.qualified_name} {usage}`", inline=False)
        await ctx.send(embed=emb)
...
async def help(ctx, command: Optional[str]):
    ...
    if command:
        ...
        if len(cmds) > 1:
            emb.description += f"\n*use* `{config['prefix']}{c.name} help` *for more information .*"
            emb.add_field(name=f"subcommands:", value=f"`{'` ⋅ `'.join(cmds)}`")
        else:
            usage = c.signature.replace('[member]', '@zt').replace('[role]', 'sped').replace('[color]', '#010101').replace('[prefix]', ';').replace('<member>', '@zt').replace('<role>', 'sped').replace('<color>', '#010101').replace('<prefix>', ';').replace('[reason]', 'retarded lol').replace('[duration]', '2m')
            if c.signature:
                emb.add_field(name=f"usage :", value=f"`{config['prefix']}{c.name} {c.signature}`", inline=False)
                emb.add_field(name=f"example :", value=f"`{config['prefix']}{c.name} {usage}`", inline=False)
            else:
                emb.add_field(name=f"usage :", value=f"`{config['prefix']}{c.name}`", inline=False)
    else:
        ...
        for cog in bot.cogs:
            ...
            cmds.sort()
            emb.description = f"*use* `{config['prefix']}help [command]` *for more information .*"
